...
fun String.digits(): String = filter { it.isDigit() }
...
fun String.ellipsize(maxLength: Int): String = if (this.length > maxLength) {
...
fun String.nl2br(): String = replace("\n", "<br>")

fun String.splitByWhitespace(): List<String> = trim().split(REGEX_WHITESPACE)

fun <T : CharSequence> T.nullIfEmpty(): T? = takeUnless { it.isEmpty() }
...
fun String.findGroupValue(regex: Regex): String? = regex.find(this)?.groupValues?.getOrNull(1)
...
fun String.isNumeric(): Boolean = all { c -> c.isDigit() }
